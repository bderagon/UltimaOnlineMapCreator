...
namespace Ultima
{
	public sealed class Map
	{
		...
		private static bool m_UseDiff, m_ClearMap;

		public static bool UseDiff { get { return m_UseDiff; } set { m_UseDiff = value; Reload(); } }

        public static bool ClearMap { get { return m_ClearMap; } set { m_ClearMap = value; } }
  ...
  //          Bitmap bmp = new Bitmap(width << 3, height << 3, PixelFormat.Format16bppRgb555);
								...
								for (int k = 0, v = 0; k < 8; ++k, v += 8)
								{
									for (int p = 0; p < 8; ++p)
									{
										...
										if (highHue == 0)
										{
											...
											{
                                                if (highstatic)
                                                    *pvData++ = pColors[highID + 0x4000]; // + 0x4000
                                                else
		...
		public unsafe Bitmap GetImage(int x, int y, int width, int height, bool statics)
		{
            Bitmap bmp = new Bitmap(width << 3, height << 3, PixelFormat.Format16bppRgb555);
            BitmapData bd = bmp.LockBits(new Rectangle(0, 0, width << 3, height << 3), ImageLockMode.WriteOnly, PixelFormat.Format16bppRgb555); //PixelFormat.Format16bppRgb555
